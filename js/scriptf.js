...
function random(){
    ...
    for (var i = 0; i < 9; i++){
        ...
        partidos[i].getElementsByTagName("span")[r].style.backgroundColor = "##0647AB";
        res[i] = ["L","E","V"][r];
    }
    container.innerHTML = res.join("\xa0\xa0");
    costoactual();
...
function send(){ //Envía quiniela al whatsapp
    ...
    const quinbara = "*Quiniela de $20 pesos de fin de semana*%0A%0A";
    whatsapptext = encodeURI(localStorage.getItem("results"));
    whatsapptext = whatsapptext.split('*').join('%0D').replace(/#/g,"");
    window.location.href = "https://wa.me/527472523098?text="+quinbara + whatsapptext;}
}
